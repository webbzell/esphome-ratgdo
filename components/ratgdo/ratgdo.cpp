...
// Just log when we receive TtcAction (e.g. when wall panel hold/release is pressed)
...
//
// WALLPANEL_ACK is from the wall panel and seems acknowledge a GDO command.
// INITIALIZING_ENABLED/INITIALIZING_DISABLED are GDO-side TTC startup messages seen after reboot,
// and progress forward by opening the door.
void RATGDOComponent::received(const TtcStateMsg msg)
{
    ...
    case TtcStateCode::INITIALIZING_DISABLED:
        ...
        break;
    default:
        return; // unrecognized byte1 - already logged above
    }

    bool was_counting = ttc_is_counting(*this->ttc_state);

    uint16_t countdown = 0;
    if (ttc_is_counting(state) || ttc_is_initializing(state)) {
